...
function preload(){
    game.load.spritesheet("heroe", "images/spritesheet.png", 130, 130);
    game.load.image("bosque", "images/fondo_seamless.jpeg");
};

// Con "create" nos permite colocar objetos en el escenario
function create(){
     //Agregar imagen de fondo que se desplaza con tile.sprite
    fondo = game.add.tileSprite(0, 0, 2268, 653, "bosque")

    //Agregar color de fondo
    game.stage.backgroundColor = "#4488AA";

    //Colocar el heroe en el escenario
    player = game.add.sprite(game.width / 2.4, 440, "heroe");     //SE pone /2, para poner el personej en medio
    
    //animaciones
    player.animations.add("der", [1, 2, 3, 4], 10, true);  //10 cuadros por segundo y true indica que la animación se va a repetir
    player.animations.add("der_rapid", [1, 2, 3, 4], 10, true); 
    player.animations.add("der_muyrapid", [1, 2, 3, 4], 10, true); 
...
function update(){
    if (derecha.isDown){
        ...
        player.animations.play("der_muyrapid");
    }
